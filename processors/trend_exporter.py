...
Tabs:
    Overview       — key stats + donut charts for each checkpoint
    Flow           — PR1 → Mid → PR2 transition table + stacked bar
    Trajectories   — student state breakdown + bar chart
    By_Group       — group-level counts across checkpoints + chart
"""
...
from utils.config import (
    ...
    TRAJECTORY_LABELS, TRAJECTORY_COLORS,
    TREND_OUTPUT_FILENAME_PATTERN, LOG_DATE_FORMAT,
)
...
class TrendExporter:
    ...
    def export(
        ...
        self._write_flow(wb, flow_df, overall)
    ...
    def _write_overview(self, wb, overall, pr1_name, mid_name, pr2_name):
        ...
        stats = [
            ...
            ("", ""),
            ("Total Unique At-Risk Students (any checkpoint)",
            ...
            ("", ""),
            (f"Recovered {pr1_name} → {mid_name}",
             overall["pr1_to_mid_recovered"]),
            (f"New at {mid_name}",              overall["new_at_midterm"]),
            (f"Recovered {mid_name} → {pr2_name}",
             overall["mid_to_pr2_recovered"]),
        ...
        for r, (label, val) in enumerate(stats, start=2):
            ws.cell(r, 1, label).font = Font(
                ...
                bold=bool(label and "Students at Risk" in label or label.startswith("Total"))
            )
            ws.cell(r, 2, val).font = Font(name="Calibri", size=11)

        ws.column_dimensions["A"].width = 45
        ws.column_dimensions["B"].width = 12

        # Three checkpoint donut charts side by side
        checkpoints = [
            (pr1_name,  overall["pr1_count"],  COLOR_PR1, "E2"),
            (mid_name,  overall["mid_count"],  COLOR_PR2, "E10"),  # reuse red for mid
            (pr2_name,  overall["pr2_count"],  COLOR_MID, "E18"),
        ]
        total = overall["total_unique_students"] or 1
        for name, count, color, anchor in checkpoints:
            img = self._make_mini_donut(name, count, total, color)
    ...
    def _write_flow(self, wb, flow_df, overall):
        ...
        new = flow_df["New"].tolist()
        x = np.arange(len(transitions))

        ax.bar(x, carried,  label="Carried Forward", color=COLOR_PR1,  alpha=0.85)
        ax.bar(x, recovered, bottom=carried, label="Recovered",  color="#2E7D32", alpha=0.85)
        ax.bar(x, new,  bottom=[c+r for c,r in zip(carried, recovered)],
               label="New Students", color=COLOR_MID, alpha=0.85)

        ax.set_xticks(x)
        ax.set_xticklabels(transitions, fontsize=10)
        ax.set_ylabel("Students", fontsize=10)
        ax.set_title("Student Flow Between Checkpoints", fontsize=12,
        ...
        ws.add_image(self._fig_to_image(fig), f"A{len(flow_df) + 5}")
    ...
    def _write_trajectories(self, wb, traj_df):
        ...
        ax.set_title("Student Trajectories Across Semester", fontsize=12,
                     fontweight="bold", color="#1F3864")
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
        plt.tight_layout()

        ws.add_image(self._fig_to_image(fig), f"A{len(traj_df) + 5}")
    ...
    def _write_groups(self, wb, group_df):
        ...
        ax.set_ylabel("Students", fontsize=10)
        ax.set_title("At-Risk Students by Group and Checkpoint", fontsize=12,
                     fontweight="bold", color="#1F3864")
        ax.legend(fontsize=9, framealpha=0.5)
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
        ...
        ws.add_image(self._fig_to_image(fig), f"A{len(group_df) + 5}")
    ...
    def _make_mini_donut(self, title, count, total, color):
        ...
        ax.text(0, 0.05, str(count), ha="center", va="center",
                fontsize=18, fontweight="bold", color="#1F3864")
        ax.text(0, -0.25, f"{pct:.1f}%", ha="center", va="center",
                fontsize=10, color="#546E7A")
        ax.set_title(title, fontsize=10, fontweight="bold", color="#1F3864", pad=6)
